...
u64 virt_to_phys(page_table* p_table, u64 virt_addr){
    ...
    for(int i = 2; i >= 0; i--){
        ...
        //if the rwe is set we are a leaf
            ...
            u64 ppn[] = {
                        (p_table_entry >> 12) & 0x1ff,
                        (p_table_entry >> 21) & 0x1ff,
                        (p_table_entry >> 30) & 0x3fffffff
            };
            //getting the different offsets dependent on level
            if(i == 0){
                return (ppn[2] << 30) | (ppn[1] << 21) | (ppn[0] << 12) | (virt_addr & 0xfff);
            }
            else if(i == 1){
                return (ppn[2] << 30) | (ppn[1] << 21) | (virt_addr & 0xfffff);
            }
            else if(i == 2){
                return (ppn[2] << 30) | (virt_addr & 0x3fffffff);
            }
            else{
                return -1;
            }
